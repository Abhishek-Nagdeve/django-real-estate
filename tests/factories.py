...
class ProfileFactory(factory.django.DjangoModelFactory):
    ...
    state = factory.LazyAttribute(lambda x: f"Maharashtra")
    is_buyer = False
    is_seller = False
    is_agent = False
    top_agent = False
    rating = factory.LazyAttribute(lambda x: faker.random_int(min=1,max=5))
    num_reviews = factory.LazyAttribute(lambda x: faker.random_int(min=0,max=20))
...
class UserFactory(factory.django.DjangoModelFactory):
    ...
    email = factory.LazyAttribute(lambda x: f"abhi@realestate.com")
